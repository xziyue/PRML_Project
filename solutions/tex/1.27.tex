...
\begin{answer}{}
	...
	\begin{align}
		...
		&= q \int_{-\infty}^{y(\bm{x})} \lvert y(\bm{x}) - t \rvert^{q-1} p(t \mid \bm{x})\ dt - q \int_{y(\bm{x})}^{+\infty} \lvert y(\bm{x}) - t \rvert^{q-1}p(t \mid \bm{x})\ dt = 0.
	\end{align}
	Therefore we can see that the stationary condition for $y(\bm{x})$ is
	\begin{gather}
		q \int_{-\infty}^{y(\bm{x})} \lvert y(\bm{x}) - t \rvert^{q-1} p(t \mid \bm{x})\ dt - q \int_{y(\bm{x})}^{+\infty} \lvert y(\bm{x}) - t \rvert^{q-1}p(t \mid \bm{x})\ dt = 0\\
		 \int_{-\infty}^{y(\bm{x})} \lvert y(\bm{x}) - t \rvert^{q-1} p(t \mid \bm{x})\ dt =  \int_{y(\bm{x})}^{+\infty} \lvert y(\bm{x}) - t \rvert^{q-1}p(t \mid \bm{x})\ dt.
	\end{gather}
